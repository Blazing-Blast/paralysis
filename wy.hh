...
namespace wy
{
    ...
    struct rand
    {
        ...
        forceinline std::vector<T> generate_stream(size_t size) noexcept
        {
            std::vector<T> result;
        ...
        template <class T = uint8_t>
        void generate_stream(std::span<T> data) noexcept
        {
    ...
    struct hash : private internal::hash_imp
    {
        ...
        forceinline uint64_t operator()(const T &elem) const noexcept
        {
            ...
            if (sizeof(T) == 16)
                return internal::wyhash64(internal::_wyr8(&elem), internal::_wyr8(reinterpret_cast<const uint8_t *>(&elem) + 8));
    ...
    template <class T>
    struct hash<std::vector<T>> : private internal::hash_imp
    {
        using hash_imp::hash_imp; // Inherit constructors
